...
export const apiFirstDesign: RoadmapLesson = {
  ...
  content: {
    ...
    howItWorks: [
      ...
      {
        type: "p",
        text: "Designing the API first forces a handful of decisions into the open *before* they quietly harden into mistakes you can't easily undo. Each bullet below is one such decision — what it is, and how to make it well:",
      },
      {
        type: "ul",
        items: [
          "**Return type — what does the method hand back?** `void` tells the caller nothing. A raw `Ticket` is fine when everything goes right, but says nothing about what happens on failure. A `Result<Ticket>` or `Optional<Ticket>` makes \"this might not produce a ticket\" visible right in the type, so the caller can't forget to handle the empty case.",
          "**Error-handling style — how does the method report failure?** Your options are: throw an exception, return `null`, return a `Result` / `Optional`, or return an error code. Any of them can work — the rule is to pick *one* and use it everywhere. Mixing styles across methods is what makes an API exhausting to call.",
          "**Naming — does the name say what the method does?** Vague verbs like `process()`, `handle()`, or `doIt()` hide intent: you have to read the body to find out what happens. Specific names like `park(vehicle)`, `cancelBooking(id)`, or `reserveSeat(...)` tell you at a glance.",
          "**Parameter shape — how does the caller pass arguments?** A long positional list like `park(plate, color, height, isElectric, level)` is easy to get out of order. Bundling the arguments into a single **parameter object** — `park(ParkRequest)` — reads clearly and lets you add fields later without breaking existing callers.",
          "**Command vs. query — does the method act, or answer?** A *command* changes state and usually returns nothing (or just a receipt). A *query* returns an answer and changes nothing. Keep them separate: a method that quietly does both surprises everyone who calls it.",
          "**Idempotency — is it safe to call twice?** If a request times out, can the caller retry `park` without parking the same car twice? Whether a repeated call is safe is part of the contract — decide it on purpose and write it down.",
          "**Minimal surface — how much do you expose?** Make public only the few methods callers actually need. Every public method is a promise you have to keep forever, so a smaller surface leaves you more freedom to change things later.",
        ],
      },
